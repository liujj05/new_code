...
namespace _2018_7_10_T
{
    public partial class Form1 : Form
    {
        // 可能需要频繁调整的参数
        // ------------------------文件处理过滤程序----------------------------
        const double data_up_lim = 355;
        const double data_down_lim = 335;
        ...
        // 启动检测延时和启动终止延时
        const int min_in_number = 45;       // #CHANGE# 连续几个点位于探测区间内时开始存数据
        const int min_sample_num = 75;     // #CHANGE# 采集此数目点之后才决定是否停止采集
        ...
        // 存储全部数据的文件变量
        StreamReader Whole_data_file = null;
        // 文件的基础名称
        ...
        // 用于数据滤波的list
        List <double> chart_data_height_mid = new List<double>();
        List <double> chart_data_height_filter = new List<double>();
        ...
        private void CrossThreadFlush()
        {
            ...
            // 数据滤波
            double dx_thresh = 1;   // 斜率阈值
            double dx1, dx2;            
            int filter_index;       // 双侧导数超限滤波的索引
            double temp_cur, temp_before, temp_next;
            double[] smooth_all = {-1, -1, -1, -1};
            int smooth_index, j1_index, j2_index;
            ...
            while (false == thread_exit)
            {
                ...
                chart_data_height_mid.Clear();
                chart_data_height_filter.Clear();
                chart_x_index.Clear();
                delta_height.Clear();
                ...
                while (true)
                {
                    ...
                    else
                    {
                        ...
                        chart_data_height_mid.Add(System.Convert.ToDouble(str));
                        chart_data_height_filter.Add(System.Convert.ToDouble(str));
                        // chart_x_index.Add(x_index); // 这回要画的不是绝对高度是相对误差量
                        x_index = x_index + 1;
                    }
                }
                chart_data_height_mid.RemoveAt((int)(x_index - 1));
                chart_data_height_mid.RemoveAt(0);
                chart_data_height_filter.RemoveAt((int)(x_index - 1));
                chart_data_height_filter.RemoveAt(0);
                Data_File.Close();

                // ========================  滤波开始  ==============================
                filter_index = 0;
                // 所有斜率过大的点设为-1
                while (filter_index < (x_index - 2))
                {
                    temp_cur = chart_data_height_mid[filter_index];
                    temp_next = chart_data_height[filter_index + 2];
                    temp_before = chart_data_height[filter_index];
                    dx1 = System.Math.Abs( temp_next - temp_cur);
                    dx2 = System.Math.Abs( temp_before - temp_cur);                    
                    if (dx1 > dx_thresh)
                        chart_data_height_filter[filter_index] = -1;
                    if (dx2 > dx_thresh)
                        chart_data_height_filter[filter_index] = -1;
                    filter_index++;
                }
                ...
                while (filter_index < (x_index - 2))
                {
                    temp_cur = chart_data_height_filter[filter_index];
                    
                    if (-1 == temp_cur)
                    {
                        smooth_index = 0;
                        j1_index = filter_index;
                        j2_index = filter_index;
                        while ( (j1_index > 0) && (smooth_index < 2) )
                        {
                            j1_index--;
                            temp_before = chart_data_height_filter[j1_index];
                            if (temp_before != -1)
                            {
                                smooth_all[smooth_index++] = temp_before;
                            }
                        }
                        smooth_index = 2;
                        while ( (j2_index < (x_index-3)) && (smooth_index < 4) )
                        {
                            j2_index++;
                            temp_next = chart_data_height_filter[j2_index];
                            if (temp_next != -1)
                            {
                                smooth_all[smooth_index++] = temp_next;
                            }
                        }
                        smooth_index = 0;
                        temp_cur = 0;
                        average_count = 0;
                        while (smooth_index < 4)
                        {
                            if (smooth_all[smooth_index] > 0)
                            {
                                temp_cur = temp_cur + smooth_all[smooth_index++];
                                average_count = average_count + 1;
                            }
                        }
                        temp_cur = temp_cur / average_count;
                        chart_data_height_filter[filter_index] = temp_cur;
                    }
                    filter_index++;
                }

                // 计算均值 - 假设标准文件也被滤波过了
                int loop_max = System.Math.Min((int)(x_index-2), standard_data_num);
                for (int i = 0; i < loop_max; i++)
                {
                    // 加入判断性语句
                    // 1. 读入高度数据
                    data_read_in = chart_data_height_filter[i];
                    // 2. 判断是否在阈值范围以内
                    if ( (data_read_in < data_up_lim) && (data_read_in > data_down_lim) )
                    {
                        // 注意：标准高度肯定比读进来的高度更大，因为更远离传感器
                        delta_temp = Standard_height[i] - chart_data_height_filter[i];
                        // 判断正负
                        if (delta_temp > 0)
                        {
        ...
        private void ReceivedData(byte[] data)
        {
            ...
            // 全流程数据保存
            Whole_data_file.WriteLine("{0}\t{1}", dHeight, dmA);
        ...
        private void Start_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                string whole_data_file_name;
                whole_data_file_name = "whole_data" + DateTime.Now.ToString();
                whole_data_file_name = whole_data_file_name.Replace(':', '_');
                whole_data_file_name = whole_data_file_name.Replace('-', '_');
                whole_data_file_name = whole_data_file_name.Replace(' ', '_');
                Whole_data_file = new StreamWriter(whole_data_file_name, false);
        ...
        private void buttonStop_Click(object sender, EventArgs e)
        {
            ...
            Whole_data_file.Flush();
            Whole_data_file.Close();
        }
