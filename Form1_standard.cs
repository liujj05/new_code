// 标准点采集程序
// 7-18实验过后，查出如下问题：
// 1. 激光所编程、工装方面均有瑕疵，在刀具加工时测量可能出现少数偏差很大的飞点，打断原有的采集过程
// 2. 两把刀加工间隙，进入信号有误触发的情况存在。

// 针对第一点：进入采集流程之后，将采集足够数目的点之后再启动退出检测流程，避免采集过程中出现的飞点干扰测量
//              但是这样一来，要相当严格的把控开始采集的位置，否则采集一旦开始将无法停止

// 针对第二点：为了避免误触发，可以将连续3点的规则加长至连续20点，以避免误触发的情况，同时进一步收紧采集的上下限
...
namespace SerialportSample
{
    public partial class SerialportSampleForm : Form
    {
        ...
        private void ReceivedData(byte[] data)
        {
            ...
            {
                if (dHeight < thresh_low) // 进入连续接近 min_in_number 个点的判断
                {
                    height_data[continue3low++] = dHeight;
                    if (continue3low == min_in_number)
                    {
                        System.Diagnostics.Debug.WriteLine("###DEBUG### - Sample START!");
            ...
            else // 采样开始
            {
                ...
                else
                {
                    ...
                    System.Diagnostics.Debug.WriteLine("###DEBUG### - ERROR! Data Number is bigger than 512 !");
                }
                ...
                if ((dHeight > thresh_high) && (height_data_num > min_sample_num)) // 进入连续远离三点的判断
                {
                    ...
                    if (continue3high == 3)
                    {
                        System.Diagnostics.Debug.WriteLine("###DEBUG### - Sample STOP!");
